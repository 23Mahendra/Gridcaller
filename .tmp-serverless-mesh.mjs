...
export function encryptText(text, secret) {
    const key = deriveKey(secret);
    const data = encoder.encode(text);
    const output = new Uint8Array(data.length + 1);
    output[0] = data.length & 0xff;
    for (let i = 0; i < data.length; i++)
        output[i + 1] = (data[i] ^ key[i % key.length]) & 0xff;
    return btoa(String.fromCharCode(...output));
}
export function decryptText(cipher, secret) {
    const key = deriveKey(secret);
    const bytes = Uint8Array.from(atob(cipher), (c) => c.charCodeAt(0));
    const len = bytes[0] || 0;
    const data = new Uint8Array(len);
    for (let i = 0; i < len; i++)
        data[i] = (bytes[i + 1] ^ key[i % key.length]) & 0xff;
    return decoder.decode(data);
}
export function createLocalMeshEnvelope(payload, secret) {
    return {
        kind: 'mesh-envelope',
...
export function readLocalMeshEnvelope(envelope, secret) {

...
class MeshCommsEngine {
  ...
  async callMeshPeer(targetPeerId: string): Promise<RTCPeerConnection | null> {
    ...
    if (this.gun) {
      ...
      this.gun.get("gridalive.webrtc.ice_remote").get(callId).map().on(async (data: any) => {
        if (!data) return;
        try {
          await this.addRemoteIceCandidate(
            pc,
            JSON.parse(data)
          );
        } catch (err) {
          console.warn(
            "[MeshComms] Gun remote ICE handling failed:",
            err
          );
        }
      });
      ...
      try {
        ...
        if (pc.signalingState !== "stable") {
          void pc.setRemoteDescription(answer)
            .then(() => this.flushRemoteIce(pc))
            .catch((err) => {
              console.warn(
                "[MeshComms] Broadcast answer SDP failed:",
                err
              );
            });
        }
    ...
    const offBroadcastIce = MeshEngine.onMessage((msg: any) => {
      ...
      try {
        void this.addRemoteIceCandidate(
          pc,
          JSON.parse(msg.data.candidate)
        );
      } catch (err) {
        console.warn(
          "[MeshComms] Broadcast remote ICE parsing failed:",
          err
        );
      }
    });
      ...
      const acceptCall = async () => {
          ...
          if (this.gun) {
            this.gun.get("gridalive.webrtc.ice").get(callId).map().on(async (candData: any) => {
              ...
              try {
                ...
                console.warn(
                  "[MeshComms] Incoming Gun ICE handling failed:",
                  err
                );
              }
            });
          ...
          const offIce = MeshEngine.onMessage((msg: any) => {
            ...
            try {
              ...
              console.warn(
                "[MeshComms] Incoming broadcast ICE parsing failed:",
                err
              );
            }
          });
  ...
  async callLocalLan(targetPeerId: string): Promise<RTCPeerConnection | null> {
    ...
    const offAnswer = MeshEngine.onMessage((msg: any) => {
      ...
      try {
        void pc.setRemoteDescription(
          ...
          .catch((err) => {
            console.warn(
              "[MeshComms] LAN broadcast answer failed:",
              err
            );
          });
      } catch (err) {
        console.warn(
          "[MeshComms] LAN broadcast answer parse failed:",
          err
        );
      }
    });
      ...
      this.gun.get("gridalive.webrtc.answer").get(callId).on((data: any) => {
        ...
        try {
          void pc.setRemoteDescription(
            ...
            .catch((err) => {
              console.warn(
                "[MeshComms] LAN Gun answer failed:",
                err
              );
            });
        } catch (err) {
          console.warn(
            "[MeshComms] LAN Gun answer parse failed:",
            err
          );
        }
      });
    ...
    const offIce = MeshEngine.onMessage((msg: any) => {
      ...
      try {
        ...
        console.warn(
          "[MeshComms] LAN remote ICE parsing failed:",
          err
        );
      }
    });
  ...
  /**
   * Centralized remote ICE handling.
   *
   * WebRTC signaling can deliver ICE candidates before
   * setRemoteDescription() has completed. In that situation
   * addIceCandidate() can fail with InvalidStateError.
   *
   * Each RTCPeerConnection gets an isolated queue.
   */
  private readonly remoteIceQueues = new WeakMap<
    RTCPeerConnection,
    RTCIceCandidateInit[]
  >();
  ...
  private async addRemoteIceCandidate(
    ...
    let normalized: RTCIceCandidateInit;
    ...
    try {
      ...
      console.warn("[MeshComms] Invalid remote ICE payload:", err);
      return;
    }

    if (!pc.remoteDescription) {
      const queue = this.remoteIceQueues.get(pc) || [];
      queue.push(normalized);
      this.remoteIceQueues.set(pc, queue);
      return;
    }

    try {
      await pc.addIceCandidate(
        new RTCIceCandidate(normalized)
      );
    } catch (err) {
      console.warn(
        "[MeshComms] Remote ICE candidate rejected:",
        err
      );
    }
  }
  ...
  private async flushRemoteIce(
    ...
    this.remoteIceQueues.delete(pc);
    ...
    for (const candidate of queue) {
      try {
        ...
        console.warn(
          "[MeshComms] Queued remote ICE candidate rejected:",
          err
        );
      }
    }
      ...
      const offCleanup = (() => {
        ...
        const handle = answerNode.on(async (data: any) => {
          ...
          try {
            ...
            // Register ICE listener BEFORE applying remote SDP.
            // Early candidates are queued by the centralized helper.
            ...
            iceNode.map().on(async (raw: string) => {
              ...
              try {
                ...
                console.warn(
                  "[MeshComms] Lone call remote ICE handling failed:",
                  err
                );
              }
            });

    ...
    <div className="grid-radio-panel">
      ...
      <section className="grid-radio-active">
        ...
        <div className="grid-radio-meters">
          ...
          <div className="grid-radio-meter rx"><strong>RX</strong><div className="meter-bars">{Array.from({length:8},(_,i)=><i key={i} className={!tx && i<Math.min(8, Number(status.rxPackets || 0) > 0 ? 6 : 0) ? "on rx" : ""}/>)}</div><Volume2 size={16}/><span>{status.rxVoicePackets ? `${status.rxVoicePackets} VOICE` : "LISTEN"}</span></div>
        </div>
        <div className="grid-radio-live-strip"><span>Node <strong>{status.radioId}</strong></span><span className="channel-live"><i className="live-dot" /> {tx ? `Sending · ${status.txPackets} frames` : `Listening · ${status.rxPackets} frames`} <Signal size={14}/></span></div>
